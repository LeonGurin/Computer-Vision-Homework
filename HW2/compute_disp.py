...
#     sigma_r, sigma_s, WMF_r = 15, 5, 15
    ...
    for d in range(max_disp+1):
        ...
        cost = np.sum(l_shift^r_shift, axis=0).astype(np.float32)
    ...
    rl_check = np.zeros((h, w), dtype=np.float32)
    x, y = np.meshgrid(range(w),range(h))
    l_x = (x + r_disp_map) # x-DL(x,y)
    mask1 = (l_x < w) # coordinate should be non-negative integer
    r_disp = r_disp_map[mask1]
    l_disp = l_disp_map[y[mask1], l_x[mask1]]
    mask2 = (l_disp == r_disp) # check if DL(x,y) = DR(x-DL(x,y))
    rl_check[y[mask1][mask2], x[mask1][mask2]] = r_disp_map[mask1][mask2]
    ...
    return lr_check, rl_check, labels
